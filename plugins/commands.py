...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton("☘️ 𝐌𝐚𝐢𝐧 𝐇𝐮𝐛 🦞", url='https://t.me/+K5AB8XAedEsyNTBl'),
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/+E0x9VQvJ2bMwNjM9"),
                        InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url="https://t.me/FILME_MAKER")
                    ],[
                        InlineKeyboardButton('Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ', url="https://t.me/+E0x9VQvJ2bMwNjM9")
                    ],[
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files_ = await get_file_details(file_id)
            files1 = files_[0]
            title ='@AutofilterV01_bot ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"@AutofilterV01_bot {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            title = '@AutofilterV01_bot ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@AutofilterV01_bot ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@AutofilterV01_bot {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
...
# Free Trail Remove ( This Code Modified By NBBotz)
